...
export default function Home() {
  ...
  useEffect(function() {
    try {
      ...
      if (!auth.loggedIn) { router.replace('/'); return; }
  ...
  const handleLogout = function() {
    localStorage.removeItem('eat_auth');
    router.push('/');
  };
  ...
  const STRATEGIES = [
    ...
    // ── Fundamental: Shareholding & Institutional ──
    { label: 'FII + DII Buying', icon: '🤝', cat: 'fundamental', query: 'Indian NSE stocks where BOTH FII and DII shareholding increased in the latest quarter — institutional confidence from both foreign and domestic investors. List top 15 with FII change, DII change, current FII%, current DII%, and promoter holding.' },
    { label: 'DII Buying', icon: '🇮🇳', cat: 'fundamental', query: 'Indian NSE stocks where DII (Domestic Institutional Investors like mutual funds, insurance companies) shareholding has increased significantly in the latest quarter. List top 15 with current DII holding, previous quarter DII holding, and the change percentage.' },
    { label: 'Promoter > 75%', icon: '👑', cat: 'fundamental', query: 'Indian NSE stocks where promoter holding is above 75%. Note: SEBI requires minimum 25% public shareholding so these companies may face selling pressure from promoter stake dilution. List top 15 with promoter holding percentage, market cap, PE, and any recent stake sale announcements.' },
    // ── Fundamental: Value & Book Value ──
    { label: 'High Book Value', icon: '📗', cat: 'fundamental', query: 'Indian NSE stocks where current price is below 100, current price is less than book value per share, debt-to-equity is below 0.2, and ROE is positive. Deep value stocks trading below book. List top 15 with price, book value, PB ratio, D/E, and ROE.' },
    { label: 'Below Book Value', icon: '📕', cat: 'fundamental', query: 'Indian NSE stocks trading below their book value — price-to-book ratio less than 1. Companies where market values them less than net asset value. List top 15 with price, book value, PB ratio, PE, and sector.' },
    { label: 'Debt Free + Below BV', icon: '🛡', cat: 'fundamental', query: 'Indian NSE stocks that are completely debt-free (debt-to-equity = 0 or near zero) AND trading below book value (PB < 1). Ultra-safe value plays. List top 15 with price, book value, PB ratio, ROE, and promoter holding.' },
    { label: 'Cash > Market Cap', icon: '🏧', cat: 'fundamental', query: 'Indian NSE stocks where cash and cash equivalents on balance sheet exceed the total market capitalization — PB below 1, PE below 5. Companies literally worth more dead than alive. List stocks with cash on books, market cap, PB, PE, and what the market is missing.' },
    { label: 'Debt Free + Dividend', icon: '🎁', cat: 'fundamental', query: 'Indian NSE companies with virtually zero debt (D/E below 0.1) that are also consistently paying dividends for at least 3 years. Financially strong companies returning cash to shareholders. List top 15 with D/E ratio, dividend yield, payout ratio, ROE, and 3-year profit CAGR.' },
    // ── Fundamental: Growth & Multibaggers ──
    { label: 'Multibagger 2025', icon: '💎', cat: 'fundamental', query: 'Indian NSE potential multibagger stocks for 2025-26 — companies with revenue between 500 to 1000 crore, strong double-digit earnings growth, expanding margins, low debt, and high ROE. Mid-cap sweet spot with more growth potential than large caps. List top 15 with revenue, profit growth, ROE, D/E, and why each could be a multibagger.' },
    { label: 'Faadoo Multibaggers', icon: '🔥', cat: 'fundamental', query: 'Indian NSE stocks with exceptional growth characteristics — sales growth above 20% for 3 years, profit growth above 25%, ROE above 20%, low D/E below 0.5, and reasonable PE. Companies with runway for massive value creation. List top 15 with all key growth metrics.' },
    { label: 'Growth for Future', icon: '🌟', cat: 'fundamental', query: 'Indian NSE future growth stocks — market cap between 1000 to 10000 crore, debt-to-equity below 0.3, PEG ratio below 0.5 (indicating growth at bargain price). Mid-caps with strong growth at attractive valuations. List top 15 with market cap, sales growth, PEG ratio, PE, and D/E.' },
    { label: 'Stock Scanner', icon: '🔍', cat: 'fundamental', query: 'Indian NSE stocks showing multibagger patterns — consistent revenue growth above 15% for 5 years, expanding operating margins, rising promoter holding, increasing institutional interest, and reasonable valuations (PE below 35). Scan for the next set of wealth creators. List top 15 with 5-year revenue CAGR, margin trend, promoter holding trend, and PE.' },
    { label: 'Grandmaster Stocks', icon: '♟', cat: 'fundamental', query: 'Indian NSE stocks using the Grandmaster strategy — after quarterly earnings season, filter stocks with double-digit quarterly revenue growth AND double-digit quarterly profit growth for at least 2 consecutive quarters, ROE above 15%, and manageable debt. Companies showing sustained earnings acceleration. List top 15 with last 2-3 quarters revenue growth, profit growth, ROE.' },
    // ── Fundamental: Zerodha & Checklist ──
    { label: 'Zerodha Checklist', icon: '✅', cat: 'fundamental', query: 'Indian NSE stocks passing the Zerodha/Varsity quality checklist — market cap above 100 crore, ROE above 18% for current year AND preceding year AND 3-year average AND 5-year average all above 18%, positive operating cash flow, sales growth above 10%, and net profit growth above 10%. Ultra-consistent quality companies. List top 15 with all ROE figures, cash flow status, and growth rates.' },
    { label: 'AI Stocks India', icon: '🤖', cat: 'fundamental', query: 'Indian NSE stocks benefiting from the AI and technology boom — companies in IT services, cloud computing, semiconductor, data centers, AI/ML services, or companies actively investing in AI capabilities. Include both pure-play tech and companies with significant AI revenue. List top 15 with market cap, revenue from tech/AI, growth rate, and PE.' },
    // ── Fundamental: Market Cap Categories ──
    { label: 'Small Cap Gems', icon: '💠', cat: 'fundamental', query: 'Indian NSE small-cap companies with market cap between 500 to 5000 crore that have strong fundamentals — ROE above 15%, sales growth above 15%, low D/E below 0.5, and positive cash flow. Quality small caps with growth potential. List top 15 with market cap, ROE, sales growth, D/E, and sector.' },
    { label: 'Best Penny Stocks', icon: '🪙', cat: 'fundamental', query: 'Indian NSE/BSE penny stocks (price below 50 rupees) with decent fundamentals — positive profits, some revenue growth, low debt, and regular trading volume above 50000. Not all penny stocks are junk. List top 15 with price, market cap, PE, revenue, profit, and D/E. Include risk warning.' },
    { label: 'Penny Below ₹2', icon: '🔬', cat: 'fundamental', query: 'Indian NSE/BSE stocks priced below 2 rupees that are debt-free or very low debt, with some business operations and positive revenue. Ultra-micro-cap deep value. List stocks with price, market cap, debt status, revenue, and a strong risk warning about penny stock volatility and liquidity risks.' },
    { label: 'Top 1000 Stocks', icon: '📋', cat: 'fundamental', query: 'List the top 20 Indian NSE stocks from the broader market (beyond Nifty 50) sorted by market capitalization — include mid-caps and emerging large-caps. Show stock name, market cap, sector, PE ratio, and 1-year return for each.' },
    { label: 'High Investment Value', icon: '📈', cat: 'fundamental', query: 'Indian NSE stocks where the market value of investments and subsidiaries on the balance sheet is very high relative to the company market cap — holding companies or conglomerates trading at significant discount to sum-of-parts value. List top 10-15 with market cap, investment value, discount percentage, and key holdings.' },
    // ── Technical: Additional ──
    { label: '200 EMA Stocks', icon: '〰️', cat: 'technical', query: 'Indian NSE stocks currently trading near their 200-day EMA (within 2-3% of 200 EMA) with good fundamentals — ROE above 12%, positive profit growth. Stocks at key support level with fundamental backing. List top 15 with current price, 200 EMA value, distance from 200 EMA, and key fundamental metrics.' },
    { label: 'Near 52W High', icon: '🏔', cat: 'technical', query: 'Indian NSE stocks currently trading within 5-10% of their 52-week high — showing strong momentum but not yet at the peak. Stocks nearing breakout to new highs. List top 15 with current price, 52-week high, distance from high, volume trend, and sector.' },
    { label: 'Swing Trading', icon: '🏄', cat: 'technical', query: 'Indian NSE stocks ideal for swing trading right now — showing RSI between 40-60 (neutral zone ready to move), price near key moving averages, recent volume pickup, and clear support/resistance levels. List top 15 with current price, RSI, key support, key resistance, and suggested entry/exit levels.' },
    { label: 'Intraday Bullish', icon: '📅', cat: 'technical', query: 'Indian NSE stocks showing bullish setup for intraday trading — positive price change in last session, positive change in last week, increasing short-term momentum, above-average volume, and price above VWAP. List top 15 with previous day change%, weekly change%, volume vs average, and suggested intraday levels.' },
  ];
  ...
  if (!authChecked) {
    return (
      <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100vh', gap:12 }}>
        <div style={{ width:32, height:32, border:'3px solid rgba(0,0,0,0.06)', borderTopColor:'var(--accent-blue)', borderRadius:'50%', animation:'spin 0.8s linear infinite' }} />
  ...
  return (
    <>
      <Head>
        <title>Equity Analysis Terminal — App</title>
      </Head>
      ...
      <div style={{
        ...
        <header style={{ padding:'28px 0 12px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            <span style={{ fontSize:13, fontWeight:500, color:'var(--text-secondary)', cursor:'pointer' }} onClick={function() { router.push('/stocks'); }}>Stocks</span>
            <span style={{ fontSize:13, fontWeight:500, color:'var(--text-secondary)', cursor:'pointer' }} onClick={function() { router.push('/about'); }}>About</span>
            <button className="btn-ghost" onClick={() => setShowWatchlist(!showWatchlist)}>
              <span style={{ fontSize:15, color:'#d97706' }}>★</span>
              Watchlist ({watchlist.length})
            </button>
            <button className="btn-ghost" onClick={handleLogout} style={{ fontSize:12, color:'var(--accent-red)' }}>Logout</button>
          </div>
        </header>
...
// Prevent static prerendering — this page needs client-side auth check
